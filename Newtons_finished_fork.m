...
for x = 1:max(xcoord)
    for y = 1:max(ycoord)
        for z = 1:max(t)
            ...
            elseif sqrt(x^2+y^2) > 51
                Ts = average(temp(x-1:x+1,y,z)+temp(x,y-1:y+1,z)); % Make the ambient temperature the average of the surrounding cells - Theoretically a boundary layer between the fluid and the surrounding should form and facilitate some heat transfer, albiet slowly
                temp(x,y,z) = Ts;
            elseif sqrt(x^2+y^2) <= 51
                Ts = average(temp(x-1:x+1,y,z)+temp(x,y-1:y+1,z));
...
% Set ambient to the average of the surrounding cells
